...
if [ -f analysis/.done_annotate ]; then
    echo "$(date) - Skipping 03_annotate_and_novel.sh (already done)" >> pipeline.log
    exit 0
fi

mkdir -p analysis
max_cpus=$(nproc)
echo "$(date) - Starting 03_annotate_and_novel.sh" >> pipeline.log
...
echo "$(date) - 03_annotate_and_novel.sh completed" >> pipeline.log
